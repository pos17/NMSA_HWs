...
c_0_fun = @(x) exp((-1/(2*mu*pi))*(1-cos(pi*xlabel)));
c_0 = integral(c_0_fun,I(1),I(2));
c_n_fun = @(n,x) exp((-1/(2*mu*pi))*(1-cos(pi*xlabel)))*cos(n*pi*x);
c_n = @(n) integral(c_n_fun,I(1),I(2));

u = @(n,x,t) 2* mu * pi * (c_n(n)*n*exp(-n^2*pi^2*mu*t)*sin(n*pi*x))/(c_0+(c_n*exp(-n^2*pi^2*mu*t)*cos(n*pi*x)));
...
for j = 1:NX+1
    for nn = 0: n_harm
        SOL_EX(j,1) =SOL_EX(j,1)+  u(nn,I(1) + (j-1)*dx,NT*dt);
    end
end
L2_ERR   = norm(SOL_EX-SOL(:,NT+1),2)*dx^0.5;
L1_ERR   = norm(SOL_EX-SOL(:,NT+1),1)*dx;
