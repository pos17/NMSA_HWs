...
[bound_pts] = C_create_bound_pts(Dati.domain,Region.coord,Dati.bc);
...
function [bound_pts] = C_create_bound_pts(domain, coord, bc)
...
if (strcmp(bc,'ND'))
    ...
    bound_pts = find(coord(:,1)==x1 );
    
elseif (strcmp(bc,'DN'))
    x0 = domain(1,1);
    bound_pts = ones(length(coord),1);
    bound_pts = find(coord(:,1)==x0);
else
    x0 = domain(1,1);
    x1 = domain(1,2);
    bound_pts = ones(length(coord),1);
