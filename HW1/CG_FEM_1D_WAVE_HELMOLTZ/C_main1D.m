...
M_mod = V.' * M * V;
A_mod = V.' * A * V;
M_mod_diag = diag(M_mod);
M_mod_tocheck = (M_mod_diag .* omega_h.^2)
check = M_mod_tocheck - diag(A_mod)
omega_range = eval(Dati.omega);
omega_linsp = linspace(omega_h(1),omega_h(end),1000)
